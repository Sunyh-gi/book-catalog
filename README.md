...
| 启用豆瓣在线搜索 | 那台设备需运行 `python _douban_server.py`（需 Python；手机不支持，用「上传封面」替代） |
...
├── data\books.js               # 静态数据源（<script src> 引入，相对位置勿动）
├── covers\<书id>.jpg           # 本地封面图（入库自动下载 / 手动放置自动生效）
...
├── _smoke.js                   # 冒烟测试（puppeteer + Edge，60 断言）
...
1. **数据两层**：`data/books.js` 是源（手改或 merge 脚本写回）；页面上的一切改动（改标记/录书/删书/题材/封面）写 **localStorage `booklib.v1` 覆盖层**，不清浏览器数据就一直生效
2. **覆盖层结构**：`{ status: {书id: owned|pending}, added: [新书], g: {list: 题材字典|null, map: {书id: genre}}, cover: {书id: "covers/x.jpg"}, deleted: [种子书id] }`
3. **计数全部从数据派生**，页面不写死数量
